"""Simulation-clock-driven rolling prediction scheduler."""
...
DEFAULT_PREDICTION_INTERVAL_SECONDS = 5.0
DEFAULT_RUN_ID_PREFIX = "PRED"


class RollingPredictionScheduler:
    """Create at most one prediction run in each simulation-time interval."""
    ...
    def __init__(
        ...
        *,
        ...
        interval_seconds: Real = DEFAULT_PREDICTION_INTERVAL_SECONDS,
        ...
        if not isinstance(service, PredictionRunService):
            raise TypeError("service must be a PredictionRunService")
        ...
        self._run_id_prefix = require_identifier(run_id_prefix, field_name="run_id_prefix")
        self._observed_reset_count = clock.reset_count
        self._last_slot: int | None = None
        self._last_run: PredictionRun | None = None
    ...
    @property
    def service(self) -> PredictionRunService:
    ...
    @property
    def last_run(self) -> PredictionRun | None:
        self._synchronize_reset()
        return self._last_run

    def run_if_due(self, snapshot: TrafficSnapshot) -> PredictionRun | None:
        """Create the current slot's run, or return None when it is not due."""
        ...
        prediction_run = self._service.run(
            snapshot,
            prediction_run_id=(f"{self._run_id_prefix}-{self._clock.tick_count:012d}"),
            generated_at_utc=snapshot.timestamp_utc,
        )
        self._last_slot = current_slot
        self._last_run = prediction_run
