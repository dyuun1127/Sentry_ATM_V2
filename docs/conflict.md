...
**현재 기본값은 규정 프로파일이다.** `sentry_atm.regulation.policy.active_separation_profile()`
이 정하며, 탐지기·위험평가기·세션 읽기모델이 모두 이 한 곳을 따른다.

| | 수평 | 수직 | 근거 |
|---|---:|---:|---|
| **규정 프로파일 (현재)** | **3 NM** | 1,000 ft | 고시 5-5-4 가 / 4-5-1 |
| `POC_TERMINAL_V1` (원본) | 5 NM | 1,000 ft | `ASM-018` 잠정 PoC 값 |

규정 프로파일은 값 하나를 고정하지 않고 **쌍별로 판정한다.** 항적난기류·착륙
최저치처럼 조항에 조건이 붙은 것은 그 조건까지 본다 — 자세한 것은
[`regulation.md`](regulation.md).

`POC_TERMINAL_V1` 은 생성자로 주입하면 그대로 쓸 수 있다. 아래 판정식은 두
프로파일 모두에 적용된다.
