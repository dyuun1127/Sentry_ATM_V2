...
| `계층` 단추 | 지형·공역·거리링·체공장주·픽스·데이터블록·**최저고도** 켜고 끄기 |
| `심볼` 단추 | 항적 기호 방식 전환 |
...
빔프로젝터에 쏘는 화면이라 **선 굵기와 이름표를 옆 패널과 따로 둔다.** 패널은
발표자가 코앞에서 보고, 스코프는 뒷줄에서 본다.

```css
--sw-hair: 1.4px   지형·거리링·인출선 — 배경
--sw-thin: 1.7px   공역 경계, 픽스, 중심선
--sw-mid:  2.2px   항적, 속도벡터, 충돌선, 활성 체공
--sw-bold: 4px     활주로

--fs-scope-label: 12px   픽스·공역·거리링 이름
--fs-scope-tag:   14px   데이터블록, 충돌선 수치
```

한 곳에 모아 두었으므로 발표장에서 쏴 보고 한 번에 조정할 수 있다. 흩어져
있으면 하나를 키울 때마다 옆의 것과 어긋난다.

**데이터블록은 서로를 피해 앉는다.** 늘 오른쪽 위에 붙이면 가까운 두 기의
블록이 겹쳐 콜사인이 서로를 지운다 — 글자를 키우면서 실제로 그렇게 됐고, 크게
만든 것이 오히려 못 읽게 됐다. 네 방향(우상·우하·좌상·좌하)을 정해진 순서로
시도해 먼저 비어 있는 자리에 앉힌다. 순서가 고정이라 같은 배치에서는 같은
자리가 나오고 화면이 매초 흔들리지 않는다. 네 자리가 다 차면 오른쪽 위로
되돌린다 — 겹치더라도 자리를 잃는 것보다 낫다.

### 최저고도 (AD 2-16)

`계층`의 「최저고도」를 켜면 관제사가 레이더 유도할 때 대조하는 최저고도 차트가
뜬다 — 경계망, 구역별 최저고도(저온보정값은 괄호), 장애물 첨두.

**표시 전용이다.** 회피안 검증의 최저고도 판정에는 쓰지 않는다 (`ASM-046`).
화면에 있는 이유는, 비상기를 낮게 복귀시킬 때 왜 그 경로가 아닌지가 숫자로
보여야 하기 때문이다.

**기본으로 꺼 둔다.** 늘 켜 두면 스코프가 숫자로 덮인다 — 필요한 것은 「이
방향으로 낮게 내려도 되는가」를 물을 때뿐이다.

원호는 잘게 나눠 그린다. 두 점을 직선으로 이으면 24 NM 호에서 눈에 보이게 꺾인다.
...
**시계가 다시 흐르는 것은 판단 연쇄가 끝났을 때뿐이다.** 회피안 상신의 다음은
또 관문(판단 대기)이라 상신만으로는 흐르지 않는다.
...
그래서 **마지막 관문만 「이어서 진행합니다」라고 적는다.** 중간 관문이 그렇게
적으면 눌렀는데 또 멈춰 있어 고장으로 읽힌다 — 실제로 그렇게 읽혔다. 그리고
대기 상자에 `✓ 상신 › 판단 › 적용` 을 그려, 시계가 멈춰 있는 동안에도 눌렀을 때
한 칸 갔다는 것이 보이게 한다.

**판단이 끝나면 스스로 이어서 진행한다.** 관문 문구가 「누르면 이어서
진행합니다」라고 적혀 있으므로 그대로 지켜야 한다 — 한동안 단계만 바꾸고 시계는
세운 채여서, 발표자가 재생을 다시 눌러야 했다.

되돌아가면 안 되는 것이 하나 있다. **사람이 세운 화면은 건드리지 않는다.**
설명하려고 일부러 멈춰 둔 것을 판단이 끝났다고 멋대로 돌리면 말하는 중에 시각이
흘러간다. 그래서 우리가 세운 것인지(`autoPaused`)를 기억하고, 재생·정지·단계
이동을 손으로 만지면 그 표시를 지운다.

단계 이동은 목표 시각까지 앞으로 감는 방식이라 **되감기가 아니다.** 뒤 단계로
가려면 처음으로 되돌린 뒤 다시 간다.

> **배경 탭은 브라우저가 타이머를 늦춘다.** 창을 뒤로 보내면 1초 폴링이 분 단위로
> 벌어져 화면이 멈춘 것처럼 보인다. 앞으로 가져오면 곧 따라온다 — 두 화면을 모두
> 보이는 자리에 두는 것이 안전하다.
