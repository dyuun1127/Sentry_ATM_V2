...
> **구현 주기.** 위 두 목록은 서술용 예시이고, 실행 결과는 `ApproachSequenceOrchestrator`
> 가 현재 상태에서 계산한다. 정렬 기준은 비행단계 우선(FINAL → APPROACH → DESCENT),
> 같은 단계 안에서는 도달시간 순이다. 거리·속도만 보면 6,000ft 에서 빠르게 강하 중인
> 항공기가 3,000ft 에 안정된 항공기보다 앞선다고 계산되는데, 접근 순서에서는 그렇지
> 않기 때문이다.
>
> 그래서 실행 결과는 `CIV-A01 > MIL-T01 > CIV-A02 > CIV-A03` 이고 `MIL-F02` 가 빠진다 —
> 시뮬레이션에서 `MIL-F02` 는 이 시점에 상승(CLIMB) 중이라 도착 열이 아니다. 목록을
> 맞추려고 상태를 바꾸지 않았다. 고정한 것은 순서의 **성질**이다: 비상기는 앞으로
> 오고, 안정된 항공기는 밀리지 않는다.
...
> **구현 주기.** 종료 판정은 `EmergencySegmentStatus` 가 두 조건을 함께 본다 —
> 비상기가 최종접근 단계에 있고(`established_on_approach`), 그 기체가 걸린 미해소
> 충돌이 없어야(`conflict_free`) 한다. 순서를 바꾼 것만으로 끝났다고 보면 비상기가
> 아직 위험한데 화면은 정상으로 돌아간다.
>
> 이를 위해 `MIL-T01` 에 T+280 최종접근 예정 상태를 추가했다. 없으면 등속 외삽으로
> 7,000ft 를 유지한 채 공항 위를 지나가고, 종료를 판정할 근거가 영원히 생기지 않는다.
